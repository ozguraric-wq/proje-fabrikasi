# Görsel kaynakları

## Porsuk Çayı / Eskişehir

- Eser: Porsuk Çayı, Eskişehir 06
- Fotoğrafçı: Zeynel Cebeci
- Tarih: 3 Şubat 2015
- [Kaynak](https://commons.wikimedia.org/wiki/File:Porsuk_%C3%87ay%C4%B1,_Eski%C5%9Fehir_06.jpg)
- [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/)
- Kullanım: `dist/assets/eskisehir-*.webp`. Yeniden boyutlandırma, WebP dönüşümü, mobil kırpma ve CSS üzerinden görünür alan kırpması. Bu uyarlamalar CC BY-SA 4.0 ile sunulur.

## Sivrihisar

- Eser: Sivrihisar from Hills 02
- Fotoğrafçı: Orberk Demir Orlu
- Tarih: 18 Haziran 2021
- [Kaynak](https://commons.wikimedia.org/wiki/File:Sivrihisar_from_Hills_02.jpg)
- [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/)
- Kullanım: `dist/assets/sivrihisar-*.webp`. Yeniden boyutlandırma, WebP dönüşümü ve CSS üzerinden görünür alan kırpması. Bu uyarlamalar CC BY-SA 4.0 ile sunulur.

## Kurumsal logolar

Valilik ve Vizyon Eskişehir 2036 logoları, kullanıcı tarafından sağlanan ve V4 Word belgesinde bulunan özgün görsellerden alınmıştır. Sadece boyut ve dosya biçimi optimizasyonu yapılmıştır. Bu logolar için CC fotoğraf lisansı iddia edilmez.

Küçük site simgesi basit PF monogramıdır; bir resmî kurum ambleminin yeniden çizimi değildir.

## Ratel Dijital giriş logosu

Kullanıcının daha önce hazırlattığı “Ratel Dijital Logo Tasarımı.png” görseli kullanıldı. Şeffaf dış boşluklar alınarak 640 piksel genişliğe küçültüldü ve kayıpsız WebP olarak saklandı. Marka biçimi, yazı ve renkler yeniden çizilmedi.
